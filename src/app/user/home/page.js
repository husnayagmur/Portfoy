      ...
      <div className="flex-1 flex flex-col h-full text-light-light-gray items-end space-y-4 font-lora">
        <div className="flex justify-between w-full flex-row">
          <div className="w-full">
            <h1 className="lg:text-5xl md:text-5xl text-2xl font-semibold py-8 text-center">
              Merhaba, ben Hüsna Yağmur.
            </h1>
            <p className="lg:text-[20px] md:text-[22px] text-[12px] font-medium text-left md:px-4 px-1 lg:px-4">
            ...
            <Link href={"/user/about"}>
              <button className='rounded-xl lg:px-12 md:px-12 px-4 lg:py-3 md:py-3 py-1 border lg:mb-0 lg:mt-5 lg:ml-5 md:ml-5 ml-1 md:mt-8 mt-4 md:mb-4 bg-black hover:bg-gray hover:text-darkdark-gray border-gray font-bold lg:text-xl md:text-xl text-xs '>
                Daha Fazlası
              </button>
            </Link>
          </div>
        ...
        <div className='grid lg:grid-cols-4 md:grid-cols-2  lg:gap-3 md:gap-4 gap-2 w-full lg:2/4 md:h-2/6 h-2/5 px-4'>
          {projects.map((project, index) => (
            ...
            >
              ...
              >
                ...
                <motion.div
                  className="absolute inset-0 flex flex-col lg:pb-4 md:pb-4 pb-6 bg-black opacity-0 text-medium-gray items-center justify-center"
                  whileHover={{ opacity: 0.8 }}
                  transition={{ duration: 0.3 }}
                >
                  <h1 className="font-semibold py-1">{project.title}</h1>
                  <p className='font-thin text-xs text-center lg:block md:block hidden'>{project.description}</p>
                </motion.div>
